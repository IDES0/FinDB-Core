...
class Index(db.Model):
    ...
    ytd_return = db.Column(db.Float)
    last_30_days_prices = db.Column(db.JSON)

    # Relationships
    sectors = db.relationship(
        'Sector', secondary=index_to_sector, backref='indexes_from_sector')
    top_stocks = db.relationship(
        'Stock', secondary=index_to_top_stocks, backref='top_indexes_from_stock')
    stocks = db.relationship(
        'Stock', secondary=index_to_stock, backref='indexes')  # Can be NULL
...
class Stock(db.Model):
    ...
    industry_key = db.Column(db.String(50), db.ForeignKey(
        'industry.industry_key'))  # FK to Industry
    sector_key = db.Column(db.String(50), db.ForeignKey('sector.sector_key'))
    last_30_days_prices = db.Column(db.JSON)

    # Relationships
    top_indexes = db.relationship(
        'Index', secondary=stock_to_top_index, backref='stocks_top_indexes')
    industries = db.relationship(
        'Industry', secondary=industry_to_top_stocks, back_populates='top_stocks')
...
class Sector(db.Model):
    ...
    top_stocks = db.relationship(
        'Stock', secondary=sector_to_top_stocks, backref='sectors_top_stocks')
...
class Industry(db.Model):
    ...
    market_cap = db.Column(db.BigInteger)
    top_stocks = db.relationship(
        'Stock', secondary=industry_to_top_stocks, back_populates='industries')
    sectors = db.relationship(
        'Sector', secondary=correlation_sector_industry, back_populates='industries')
